...
import org.hibernate.aql.internal.ast.tree.AggregateNode;
import org.hibernate.aql.internal.ast.tree.AssignmentSpecification;
import org.hibernate.aql.internal.ast.tree.CollectionFunction;
import org.hibernate.aql.internal.ast.tree.ConstructorNode;
import org.hibernate.aql.internal.ast.tree.DeleteStatement;
import org.hibernate.aql.internal.ast.tree.PathSeparatorNode;
import org.hibernate.aql.internal.ast.tree.DotNode;
import org.hibernate.aql.internal.ast.tree.FromClause;
import org.hibernate.aql.internal.ast.tree.FromElement;
import org.hibernate.aql.internal.ast.tree.FromElementFactory;
import org.hibernate.aql.internal.ast.tree.FromReferenceNode;
import org.hibernate.aql.internal.ast.tree.IdentNode;
import org.hibernate.aql.internal.ast.tree.IndexNode;
import org.hibernate.aql.internal.ast.tree.InsertStatement;
import org.hibernate.aql.internal.ast.tree.IntoClause;
import org.hibernate.aql.internal.ast.tree.MethodNode;
import org.hibernate.aql.internal.ast.tree.OperatorNode;
import org.hibernate.aql.internal.ast.tree.ParameterContainer;
import org.hibernate.aql.internal.ast.tree.ParameterNode;
import org.hibernate.aql.internal.ast.tree.QueryNode;
import org.hibernate.aql.internal.ast.tree.ResolvableNode;
import org.hibernate.aql.internal.ast.tree.RestrictableStatement;
import org.hibernate.aql.internal.ast.tree.ResultVariableRefNode;
import org.hibernate.aql.internal.ast.tree.SelectClause;
import org.hibernate.aql.internal.ast.tree.SelectExpression;
import org.hibernate.aql.internal.ast.tree.UpdateStatement;
import org.hibernate.aql.internal.ast.util.ASTPrinter;
import org.hibernate.aql.internal.ast.util.ASTUtil;
import org.hibernate.aql.internal.ast.util.AliasGenerator;
import org.hibernate.aql.internal.ast.util.JoinProcessor;
import org.hibernate.aql.internal.ast.util.LiteralProcessor;
import org.hibernate.aql.internal.ast.util.NodeTraverser;
import org.hibernate.aql.internal.ast.util.SessionFactoryHelper;
import org.hibernate.aql.internal.ast.util.SyntheticAndFactory;
import org.hibernate.aql.spi.QueryTranslator;
import org.hibernate.engine.internal.JoinSequence;
import org.hibernate.engine.internal.ParameterBinder;
import org.hibernate.engine.spi.SessionFactoryImplementor;
import org.hibernate.aql.internal.antlr.AqlSqlBaseWalker;
import org.hibernate.aql.internal.antlr.AqlSqlTokenTypes;
 ...
 * @see SqlASTFactory
 */
public class AqlSqlWalker extends AqlSqlBaseWalker implements ErrorReporter, ParameterBinder.NamedParameterSource {

    private static final CoreMessageLogger LOG = Logger.getMessageLogger(CoreMessageLogger.class, AqlSqlWalker.class.getName());

	private final QueryTranslatorImpl queryTranslatorImpl;
	private final AqlParser hqlParser;
...
//			}
			...
			if ( isFilter() ) {
				...
				AST fromElement = ASTUtil.create( inputAstFactory, AqlTokenTypes.FILTER_ENTITY, collectionElementEntityName );
				ASTUtil.createSibling( inputAstFactory, AqlTokenTypes.ALIAS, "this", fromElement );
	...
	protected AST lookupNonQualifiedProperty(AST property) throws SemanticException {
		...
		return lookupProperty( syntheticDotNode, false, getCurrentClauseType() == AqlSqlTokenTypes.SELECT );
	}
...
//		}
    ...
    protected void postProcessInsert(AST insert) throws SemanticException, QueryException {
		...
		if ( !insertStatement.getIntoClause().isExplicitIdInsertion() ) {
			...
			if ( fragment != null ) {
                ...
                AST fragmentNode = getASTFactory().create( AqlSqlTokenTypes.SQL_TOKEN, fragment );
		...
		if ( includeVersionProperty ) {
			...
			if ( sessionFactoryHelper.getFactory().getDialect().supportsParametersInInsertSelect() ) {
				...
				versionValueNode = getASTFactory().create( AqlSqlTokenTypes.PARAM, "?" );
				...
				if ( sessionFactoryHelper.getFactory().getDialect().requiresCastingOfParametersInSelectClause() ) {
					...
					MethodNode versionMethodNode = ( MethodNode ) getASTFactory().create( AqlSqlTokenTypes.METHOD_CALL, "(" );
					AST methodIdentNode = getASTFactory().create( AqlSqlTokenTypes.IDENT, "cast" );
					versionMethodNode.addChild( methodIdentNode );
					versionMethodNode.initializeMethodNode(methodIdentNode, true );
					AST castExprListNode = getASTFactory().create( AqlSqlTokenTypes.EXPR_LIST, "exprList" );
				...
				if ( isIntegral( versionType ) ) {
					try {
						...
						versionValueNode = getASTFactory().create( AqlSqlTokenTypes.SQL_TOKEN, seedValue.toString() );
					}
					catch( Throwable t ) {
					...
					versionValueNode = getASTFactory().create( AqlSqlTokenTypes.SQL_TOKEN, functionName );
				}
				else {
		...
		if ( insertStatement.getIntoClause().isDiscriminated() ) {
			...
			AST discrimValue = getASTFactory().create( AqlSqlTokenTypes.SQL_TOKEN, sqlValue );
    ...
    protected void prepareVersioned(AST updateNode, AST versioned) throws SemanticException {
		...
		if ( versioned != null ) {
			...
			AST eq = getASTFactory().create( AqlSqlTokenTypes.EQ, "=" );
			...
			if ( isTimestampBasedVersion( versionType ) ) {
				versionIncrementNode = getASTFactory().create( AqlSqlTokenTypes.PARAM, "?" );
				ParameterSpecification paramSpec = new VersionTypeSeedParameterSpecification( versionType );
				...
				versionIncrementNode = getASTFactory().create( AqlSqlTokenTypes.PLUS, "+" );
				versionIncrementNode.setFirstChild( generateVersionPropertyNode( persister ) );
				versionIncrementNode.addChild( getASTFactory().create( AqlSqlTokenTypes.IDENT, "1" ) );
			}
	...
	private AST generateVersionPropertyNode(Queryable persister) throws SemanticException {
		...
		AST versionPropertyRef = getASTFactory().create( AqlSqlTokenTypes.IDENT, versionPropertyName );
