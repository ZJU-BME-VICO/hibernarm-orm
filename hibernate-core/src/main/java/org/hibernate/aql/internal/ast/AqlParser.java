...
import org.hibernate.aql.internal.antlr.AqlBaseParser;
 ...
 */
public final class AqlParser extends AqlBaseParser {
	private static final CoreMessageLogger LOG = Logger.getMessageLogger(
			CoreMessageLogger.class,
			AqlParser.class.getName()
	);
	...
	public static AqlParser getInstance(String hql) {
		return new AqlParser( hql );
	}

	private AqlParser(String hql) {
		// The fix for HHH-558...
		super( new AqlLexer( new StringReader( hql ) ) );
		parseErrorHandler = new ErrorCounter( hql );
		// Create nodes that track line and column number.
		setASTFactory( new AqlASTFactory() );
	}
		...
		if ( token instanceof AqlToken ) {
			...
			if ( hqlToken.isPossibleID() && ( ex instanceof MismatchedTokenException ) ) {
				...
				if ( mte.expecting == AqlTokenTypes.IDENT ) {
					...
					token.setType( AqlTokenTypes.WEIRD_IDENT );
